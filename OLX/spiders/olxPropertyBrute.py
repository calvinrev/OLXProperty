...
class OlxpropertybruteSpider(scrapy.Spider):
    name = 'olxPropertyBrute'
    ...
    def __init__(self):
        ...
        # log file handler
        fileHandler = logging.FileHandler(f'logs/log-brute-{datetime.now().strftime("%Y-%m-%d")}.log')
    ...
    def parse(self, response):
        #Setup (assumed) max number of Subdistrict (brute force)
        g5 = 5007999

        #Loop getting API response
        for batch in batchProcess([i for i in range(5000001,g5)]): #batch processing
            for loc in batch:
